          ...
          try {
            if (session == null) {
              await ApiClient.instance.dio
                  .post('/admin/sessions/', data: data);
            } else {
              await ApiClient.instance.dio
                  .patch('/admin/sessions/${session['id']}/', data: data);
            }
...
class _SessionCard extends StatelessWidget {
  ...
  const _SessionCard(
      {required this.session, required this.index, required this.onEdit});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: isCurrent ? IceColors.navyDeep.withAlpha(60) : IceColors.border),
      ),
      child: Row(children: [
        Container(
          width: 42,
          ...
          decoration: BoxDecoration(
            color:
                isCurrent ? IceColors.lime : IceColors.navyDeep.withAlpha(12),
            borderRadius: BorderRadius.circular(12),
          ),
          alignment: Alignment.center,
          child: Icon(Icons.calendar_today_rounded,
              color: isCurrent ? IceColors.navy : IceColors.navyDeep, size: 20),
        ),
              ...
              children: [
                ...
                if (isCurrent)
                  const Text('Current Session',
                      style: TextStyle(fontSize: 11, color: IceColors.navyDeep)),
              ]),
        ),
        if (isCurrent)
          Container(
