...
class _AdminLeaveScreenState extends ConsumerState<AdminLeaveScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      try {
        final res =
            await ApiClient.instance.dio.get('/admin/leave-requests/');
        data = res.data;
      } catch (_) {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor:
                isApproved ? IceColors.success : IceColors.danger,
...
class _LeaveCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final userName = request['student_name'] ??
        request['staff_name'] ??
        request['user_name'] ??
        request['user']?['name'] ??
        request['applicant'] ??
        'Unknown';
    final fromDate = request['from_date'] ??
        request['start_date'] ??
        request['date'] ??
        '';
