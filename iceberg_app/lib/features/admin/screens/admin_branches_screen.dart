...
class _AdminBranchesScreenState extends ConsumerState<AdminBranchesScreen> {
  ...
  Future<void> _load() async {
    try {
      final res =
          await ApiClient.instance.dio.get('/admin/branches-manage/');
      setState(() {
  ...
  void _showForm({Map<String, dynamic>? branch}) {
    final nameCtrl =
        TextEditingController(text: branch?['name']?.toString() ?? '');
    final addrCtrl =
      ...
      builder: (ctx) => _BranchFormSheet(
          ...
          try {
            if (branch == null) {
              await ApiClient.instance.dio
                  .post('/admin/branches-manage/', data: data);
            } else {
              await ApiClient.instance.dio
                  .patch('/admin/branches-manage/${branch['id']}/', data: data);
            }
        ...
        onDelete: branch == null
            ...
            : () async {
                try {
                  await ApiClient.instance.dio
                      .delete('/admin/branches-manage/${branch['id']}/');
                  if (ctx.mounted) Navigator.pop(ctx);
                  _load();
                } catch (_) {}
              },
  ...
  Widget _buildEmpty() {
    return const Padding(
      ...
      child: Center(
        child: Column(
          children: [
            Icon(Icons.location_city_outlined, size: 48, color: IceColors.muted),
            SizedBox(height: 12),
...
class _BranchCard extends StatelessWidget {
  ...
  const _BranchCard(
      {required this.branch, required this.index, required this.onEdit});
        ...
        children: [
          ...
          Text(title,
              style: const TextStyle(
                  ...
                  color: IceColors.text)),
          const SizedBox(height: 16),
          TextField(controller: nameCtrl, decoration: _inputDeco('Branch Name')),
          const SizedBox(height: 12),
