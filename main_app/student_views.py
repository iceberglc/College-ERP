...
def student_home(request):
        ...
        subject_rows.append(
            {
                ...
                "room": group.room,
                "teacher": group.teacher.admin.get_full_name()
                if group.teacher and group.teacher.admin
                else "",
    ...
    stories_qs = (
        DashboardStory.objects.filter(is_active=True)
    ...
    if _my_rank_entry:
        ...
        if hero_rank == 1:
            ...
            hero_tier = "cyan"
            hero_icon = "🚀"
            hero_label = f"Top {int(top_pct * 100)}%" if top_pct <= 0.5 else "Climbing"
    else:
        hero_rank = None
        hero_rank_score = 0
        hero_rank_total = 0
    ...
    qa_new_vocab_days = VocabularyDay.objects.filter(
...
def _recent_assignments(student):
    enrolled_groups = Enrollment.objects.filter(student=student, is_active=True).values_list(
        "group_id", flat=True
    )
    assignments = (
...
def _scores_trend(student):
    ...
    results = (
        StudentResult.objects.filter(student=student).select_related("group").order_by("created_at")
    )
...
def _vocab_quiz_trend(student):
    ...
    quizzes = VocabularyQuizResult.objects.filter(student=student).order_by("taken_at")
...
def student_view_attendance(request):
    student = get_object_or_404(Student, admin=request.user)
    enrolled_group_ids = Enrollment.objects.filter(student=student, is_active=True).values_list(
        "group_id", flat=True
    )
    ...
    try:
        ...
        attendance_qs = Attendance.objects.filter(group=group, date__range=(start_date, end_date))
        reports = AttendanceReport.objects.filter(
...
def student_result_files(request):
    ...
    files = (
        ResultFile.objects.filter(group_id__in=enrolled_group_ids)
...
def _peer_student_ids(student, scope):
    ...
    if scope == "group":
        my_groups = Enrollment.objects.filter(student=student, is_active=True).values_list(
            "group_id", flat=True
        )
        return (
            Enrollment.objects.filter(group_id__in=my_groups, is_active=True)
    ...
    if scope == "branch":
        ...
        branch_ids = (
            Group.objects.filter(id__in=my_groups).values_list("branch_id", flat=True).distinct()
        )
        if not branch_ids:
        ...
        return (
            Enrollment.objects.filter(group_id__in=branch_groups, is_active=True)
            .values_list("student_id", flat=True)
            .distinct()
        )
    # 'all'
...
def _rank_score(student_id, time_start, weights, enrolled_by_student):
    ...
    attendance = round(att_agg["good"] / att_agg["total"] * 100) if att_agg["total"] else None
...
def _build_student_rankings(student, scope, time_filter):
    ...
    enrolled_by_student = {}
    for e in Enrollment.objects.filter(student_id__in=peer_ids, is_active=True).values(
        "student_id", "group_id"
    ):
        enrolled_by_student.setdefault(e["student_id"], []).append(e["group_id"])

    peers = Student.objects.filter(id__in=peer_ids).select_related("admin", "course")
...
def _build_group_rankings(student, scope, time_filter):
    ...
    if scope == "branch":
        my_group_ids = Enrollment.objects.filter(student=student, is_active=True).values_list(
            "group_id", flat=True
        )
        branch_ids = (
            Group.objects.filter(id__in=my_group_ids).values_list("branch_id", flat=True).distinct()
        )
    ...
    student_ids = list(
        Enrollment.objects.filter(group__in=candidate_groups, is_active=True)
        .values_list("student_id", flat=True)
        .distinct()
    )
    if not student_ids:
    ...
    for g in candidate_groups:
        members = Enrollment.objects.filter(group=g, is_active=True).values_list(
...
def _build_branch_rankings(student, scope, time_filter):
    ...
    my_branch_ids = set(
        Group.objects.filter(id__in=my_group_ids).values_list("branch_id", flat=True)
    )

    # Pre-compute all relevant student scores
    all_student_ids = list(
        Enrollment.objects.filter(group__branch__in=branches_qs, is_active=True)
        .values_list("student_id", flat=True)
        .distinct()
    )
    ...
    enrolled_by_student = {}
    for e in Enrollment.objects.filter(student_id__in=all_student_ids, is_active=True).values(
        "student_id", "group_id"
    ):
        enrolled_by_student.setdefault(e["student_id"], []).append(e["group_id"])
    ...
    weights = _leaderboard_weights()
    student_score = {
        sid: _rank_score(sid, time_start, weights, enrolled_by_student)["score"]
...
def student_leaderboard_history(request):
    ...
    student = get_object_or_404(Student, admin=request.user)
    seasons = (
        LeaderboardSeason.objects.filter(snapshots__isnull=False).distinct().order_by("-start_date")
    )
...
def student_leaderboard_season(request, season_id):
    ...
    snapshots = season.snapshots.select_related("student__admin", "student__course").order_by(
        "rank"
    )
    rows = []
    for sn in snapshots:
...
def student_assignments(request):
    ...
    assignments = (
        Assignment.objects.filter(group__in=enrolled_groups)
        .select_related("subject", "group", "created_by__admin")
        .order_by("due_date")
    )
    submitted_ids = set(
        Submission.objects.filter(student=student).values_list("assignment_id", flat=True)
...
def submit_assignment(request, assignment_id):
    ...
    if (
        ...
        return HttpResponseForbidden(
            "You are not enrolled in the group this assignment belongs to."
        )
...
def vocabulary_day_list(request):
    ...
    days = (
        VocabularyDay.objects.filter(
            models.Q(group_id__in=enrolled_group_ids)
            | models.Q(release_scope=VocabularyDay.SCOPE_ALL),
            release_at__lte=now,
        )
...
def vocabulary_day_detail(request, day_id):
    ...
    best_quiz = (
        VocabularyQuizResult.objects.filter(student=student, day=day).order_by("-score").first()
    )
    return render(
        request,
