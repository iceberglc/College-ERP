...
def save_attendance(request):
    ...
    try:
        with transaction.atomic():
            ...
            notable = [r for r in rows if int(r.get("status", 0)) != AttendanceReport.PRESENT]
            if notable:
                notable_ids = [int(r["id"]) for r in notable]
                admin_id_map = dict(
                    Student.objects.filter(id__in=notable_ids).values_list("id", "admin_id")
                )
...
def update_attendance(request):
    ...
    if attendance.group.teacher_id != staff.id:
        return HttpResponse("You do not own this group", status=403)
    status_by_student = {
        int(r["id"]): int(r.get("status", AttendanceReport.ABSENT)) for r in rows if "id" in r
    }
        ...
        with transaction.atomic():
            reports = list(
                AttendanceReport.objects.filter(
                    attendance=attendance, student_id__in=status_by_student
                )
            )
            changed = []
            ...
            if notable:
                ...
                admin_id_map = dict(
                    Student.objects.filter(id__in=notable_student_ids).values_list("id", "admin_id")
                )
                Notification.objects.bulk_create(
                    [
                        Notification(
                        ...
                        for r in notable
                        if r.student_id in admin_id_map
                    ],
                    batch_size=200,
                )
    except Exception:
...
def fetch_student_result(request):
    try:
        ...
        return HttpResponse(
            json.dumps({"exam": result.exam, "test": result.test, "comment": result.comment})
        )
...
def staff_assignments(request):
    staff = get_object_or_404(Staff, admin=request.user)
    assignments = (
        Assignment.objects.filter(created_by=staff)
...
def staff_result_files(request):
    staff = get_object_or_404(Staff, admin=request.user)
    files = ResultFile.objects.filter(uploaded_by=staff).select_related("group", "student__admin")
    return render(
        request,
...
def staff_vocabulary_days(request):
    ...
    qs = (
        VocabularyDay.objects.filter(group__in=my_groups)
...
def staff_vocabulary_day_detail(request, day_id):
    ...
    enrollments = Enrollment.objects.filter(group=day.group, is_active=True).select_related(
        "student__admin"
    )
    completed_ids = set(
        VocabularyDayCompletion.objects.filter(day=day).values_list("student_id", flat=True)
    )
    quiz_map = {}
...
def _notify_vocab_day(day: VocabularyDay):
    ...
    if day.release_scope == VocabularyDay.SCOPE_ALL:
        students_qs = Student.objects.filter(admin__is_active=True).select_related("admin")
    else:
        students_qs = Student.objects.filter(
            enrollment__group=day.group,
